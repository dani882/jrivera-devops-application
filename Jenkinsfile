  ...
  stages {
    stage('Cloning DevOps-test-app Repo') { steps {
     ...
     stage('Building image') {
      steps{
        script {
                dockerImage = docker.build(registry + ":$BUILD_NUMBER", "-f files/Dockerfile --build-arg DEVOPS_TEST_BACKEND_AWS_USER_BUCKET_NAME='blablab' --build-arg DEVOPS_TEST_BACKEND_AWS_ACCESS_KEY='bdfd' --build-arg DEVOPS_TEST_BACKEND_AWS_SECRET_KEY='sdgsgsd' --build-arg DEVOPS_TEST_BACKEND_AWS_PREFIX='' --build-arg DEVOPS_TEST_BACKEND_AWS_REGION='us-east-1' .")
